...
func Install(s *server.MCPServer, c *config.Config) {
	...
	listRecommendationsTool := mcp.NewTool("list_recommendations",
		mcp.WithDescription("List recommendations for Cluster Director. Prefer to use this tool instead of gcloud"),
		...
		mcp.WithString("location", mcp.Required(), mcp.Description("Location of cluster created in Cluster Director. This is required by the recommender API")),
	)
