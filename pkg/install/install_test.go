...
func TestGeminiCLIExtension(t *testing.T) {
	...
	testExePath := "/usr/local/bin/cluster-director-mcp"
	if err := GeminiCLIExtension(tmpDir, testVersion, testExePath); err != nil {
		t.Fatalf("GeminiCLIExtension() failed: %v", err)
	}

	extensionDir := filepath.Join(tmpDir, ".gemini", "extensions", "cluster-director-mcp")
  ...
  "mcpServers": {
    "cluster-director": {
      "command": "/usr/local/bin/cluster-director-mcp"
    }
  }
}`
